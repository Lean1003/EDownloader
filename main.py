    ...
    try:
        ...
        for i, chap in enumerate(course_data.get('chapters', [])):
            ...
            for j, less in enumerate(chap.get('lessons', [])):
                ...
                less_title = sanitize_filename(f"{j+1:02d} - {less_info.get('title', 'Less')}")
...
def main(page: ft.Page):
    ...
    # --- MODIFICATION: Added session variable for downloader stop flag ---
    ...
    def bulk_upload_worker(folder_path, start_button):
        try:
            ...
            videos_to_upload = [f for f in os.listdir(folder_path) if f.lower().endswith((".mp4", ".mov", ".mkv", ".avi", ".webm"))]
            if not videos_to_upload: print("No video files found in the selected directory."); return
            print(f"Found {len(videos_to_upload)} videos. Starting bulk upload...")
            for i, filename in enumerate(videos_to_upload):
                video_path, video_title = os.path.join(folder_path, filename), Path(filename).stem
                print(f"\n--- Uploading video {i+1} of {len(videos_to_upload)}: {filename} ---")
                if not upload_video(yt, cid, video_path, video_title, f"Uploaded via Flet Bulk Uploader.", youtube_upload_progress_ring, youtube_upload_button):
                    print(f"Stopping bulk upload due to an error with '{filename}'."); break
    ...
    # --- MODIFICATION: Updated downloader_worker to handle stop logic ---
    def downloader_worker(json_path, token, output_dir, start_button, stop_button, progress_bar):
        try:
            ...
            else: # This 'else' belongs to the 'for' loop, executing only if the loop completes without 'break'
                print("\nTải khóa học hoàn tất!")

        except Exception as e: print(f"❌ Lỗi nghiêm trọng trong downloader: {e}")
        finally:
            page.session.set("downloader_should_stop", False) # Reset flag
            start_button.visible = True
            stop_button.visible = False
            downloader_manage_log_button.disabled = False
    ...
    # --- MODIFICATION: Updated start_downloader_flow to manage button states ---
    def start_downloader_flow(e):
        ...
        page.session.set("downloader_should_stop", False) # Reset flag before starting
        e.control.visible = False
        downloader_stop_button.visible = True
        downloader_manage_log_button.disabled = True
        page.update()
    ...
    def on_downloader_json_picked(e: ft.FilePickerResultEvent):
        ...
        page.update()
    # --- MODIFICATION: Added handler for the new stop button ---
    def stop_downloader_flow(e):
        print("Stop command received. The downloader will stop after the current file...")
        page.session.set("downloader_should_stop", True)
        e.control.disabled = True # Prevent multiple clicks
        page.update()
    ...
    youtube_view = ft.Column([ft.Text("YouTube Uploader",size=24,weight=ft.FontWeight.BOLD),ft.Text("Step 1: Authenticate"),youtube_auth_stack,youtube_selected_channel_text,ft.Divider(),ft.Text("Step 2: Upload Single Video",size=20),youtube_title_field,youtube_description_field,ft.Row([ft.ElevatedButton("Select Video",on_click=lambda _:youtube_file_picker.pick_files(allowed_extensions=["mp4","mov"])),youtube_selected_file_text]),ft.Row([youtube_upload_button,youtube_upload_progress_ring]),ft.Divider(),ft.Text("Step 3: Bulk Upload from Folder",size=20),ft.Row([ft.ElevatedButton("Select Folder",on_click=lambda _:directory_picker.get_directory_path(),icon=ft.icons.FOLDER_OPEN),bulk_upload_folder_text]),bulk_upload_button],spacing=12,scroll=ft.ScrollMode.AUTO)
    ...
    # --- MODIFICATION: Added Downloader Stop Button and placed buttons in a Stack for toggling ---
